...
public class Trab4_3_BrilhoContrasteSolarizacao implements PlugIn, DialogListener {
	...
	public void GenerateWindow() {
		...
		janela.addMessage("Esse PlugIn irá ajustar o Brilho, Contraste e Solarização da imagem.");
	...
	public boolean dialogItemChanged(GenericDialog janela, AWTEvent e) {
		...
		AdjustImage(brilho, contraste, solarizacao);

		return true;
	}

	public void AdjustImage(int brilho, float contraste, int[] solarizacao) {
		int pixel[] = new int[3];
...
//		ImageProcessor processorBackup = imgBackup.getProcessor();
		...
		for (int x = 0; x < processorOriginal.getWidth(); x++) {
			for (int y = 0; y < processorOriginal.getHeight(); y++) {
				...
				processorOriginal.putPixel(x, y, Pixel_Converter(pixel, brilho, contraste, solarizacao));
			}
		}
	...
	private int[] Pixel_Converter(int[] pixel, int b, float c, int[] solarizacao) {
		...
		// System.out.println("Brightnesses value: R%d G%d B%d".formatted(pixel[0], pixel[1], pixel[2]));
		...
		// System.out.println("Contrasted value: Factor(%.2f) R%d G%d B%d".formatted(factor, pixel[0], pixel[1], pixel[2]));
		...
		// System.out.println("Solarized value: R%d G%d B%d".formatted(pixel[0], pixel[1], pixel[2]));
